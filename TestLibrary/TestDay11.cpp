...
namespace TestLibrary
{
	...
	public:
		...
		TEST_METHOD(First)
		{
			Days::Day11 day(input);

			auto resultEx = UnwrapResult<Days::DayIntegerResult>(
				[&day]() { return day.runFirst(true); 
			});

			Assert::AreEqual(10605u, resultEx.first->Value, L"Example data has wrong result");
		...
		TEST_METHOD(Second)
		{
			...
			auto resultEx = UnwrapResult<Days::DayBigIntegerResult>(
				[&day]() { return day.runSecond(true);
			});
			
			Assert::AreEqual(2713310158ull, resultEx.first->Value, L"Example data has wrong result");

			auto result = UnwrapResult<Days::DayBigIntegerResult>(
				[&day]() { return day.runSecond(false);
			});


			Logger::WriteMessage(Shared::WideStrUL(resultEx.first->Value).c_str());
			Logger::WriteMessage(Shared::WideStrUL(result.first->Value).c_str());
		}
	};
}
