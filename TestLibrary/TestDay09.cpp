...
namespace TestLibrary
{
	...
	public:
		...
		TEST_METHOD(First)
		{
			Days::Day09 day(input);

			auto resultEx = UnwrapResult<Days::DayIntegerResult>(
				[&day]() { return day.runFirst(true); 
			});

			Assert::AreEqual(13u, resultEx.first->Value, L"Example data has wrong result");
		...
		TEST_METHOD(Second)
		{
			...
			auto resultEx = UnwrapResult<Days::DayIntegerResult>(
				[&day]() { return day.runSecond(true, 2);
			});
			
			Assert::AreEqual(36u, resultEx.first->Value, L"Example data has wrong result");
